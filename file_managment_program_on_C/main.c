...
void list_files(){
    ...
    dir=opendir(cwd);
    if(dir==NULL)printf("Unable to open directory.\n");
...
void change_directory(){
    ...
    scanf("%s",path);
    if(chdir(path)==0)printf("Changed directory to: %s\n",path);
    else printf("Failed to change directory.\n");}
void create_directory(){
    char dirname[con];
    printf("Enter new directory name: ");
    scanf("%s",dirname);
    if(mkdir(dirname,0777)==0)printf("Directory '%s' created successfully.\n",dirname);
...
void create_file() {
    ...
    if (file) {printf("File '%s' created successfully.\n", filename);
    fclose(file);}else perror("Error creating file");}
int main(){
    int choice;
    while(1){
        printf("\n--- CLI File Manager ---\n1. List files\n2. Change directory\n3. Create directory\n4. Delete file\n5. Create file\n6. Quit\nEnter your choice: ");
        ...
        switch(choice){
            ...
            case 2:change_directory();break;
            case 3:create_directory();break;
            case 4:delete_file();break;
            case 5:create_file();break;
